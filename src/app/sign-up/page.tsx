...
import { signUp, signInWithGoogle } from "@/src/store/auth/authSlice";
    ...
    try {
      const idToken = await new Promise<string>((resolve, reject) => {
        ...
        const listener = (e: MessageEvent) => {
          ...
          if (e.data?.state && storedState && e.data.state !== storedState)
            return;
          ...
          if (e.data?.id_token) {
            ...
            window.removeEventListener("message", listener);
            cleanup();
            resolve(e.data.id_token as string);
          }
          if (e.data?.error) {
            window.removeEventListener("message", listener);
            cleanup();
      ...
      <AnimatedSection animate="scale" className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            Start sharing securely in seconds
          </p>
        </div>
